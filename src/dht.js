 ...
 */
class KBucket {
  constructor(localNodeId, prefix = '', prefixLength = 0, debug = false, k = DEFAULT_K) {
  ...
  add(node) {
    ...
    if (this.nodes.length < this.K) {  // Use instance K value instead of constant
   ...
   */
  getClosestNodes(targetId, count = null) {  // Make count parameter optional
    ...
    if (this.left && this.right) {
      ...
      if (nodes.length < k) {
        nodes = nodes.concat(second.getClosestNodes(targetId, k - nodes.length));
      }
      return nodes;
    }
...
class DHT extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    this.REPLICATE_INTERVAL = options.replicateInterval || DEFAULT_REPLICATE_INTERVAL;
    this.REPUBLISH_INTERVAL = options.republishInterval || DEFAULT_REPUBLISH_INTERVAL;
  ...
  async _initialize(options) {
    try {
      ...
      this.buckets = Array(this.BUCKET_COUNT).fill().map(() => 
        new KBucket(this.nodeId, '', 0, this.debug, this.K));
      ...
      console.log(`DHT node created with ID: ${this.nodeIdHex}, maxPeers: ${this.maxPeers}`);
  ...
  async _rebalancePeers(newPeerId) {
    ...
    if (this._shouldReplacePeer(newPeerId, furthestPeerId)) {
      this._logDebug(`Replacing further peer ${furthestPeerId} with closer peer ${newPeerId}`);
  ...
  async connect(peerInfo) {
    ...
    if (this.peers.size >= this.maxPeers) {
      ...
      if (furthestPeer && this._shouldReplacePeer(peerId, furthestPeer)) {
        ...
        throw new Error('Max peers reached and new peer not closer than existing peers');
      }
    }
  ...
  signal(data) {
    ...
    if (this.peers.size >= this.maxPeers) {
      if (!this._rebalancePeers(peerId)) {
        this._logDebug(`Max peers (${this.maxPeers}) reached and ${peerId} not closer than existing peers`);
  ...
  async findNode(targetId) {
    ...
    while (nodes.length > 0) {
      const nodesToQuery = [];
      for (let i = 0; i < nodes.length && nodesToQuery.length < this.ALPHA; i++) {
        const node = nodes[i];
        const nodeIdHex = typeof node.id === 'string' ? node.id : bufferToHex(node.id);
  ...
  async put(key, value) {
    ...
    if (this.storage.size >= this.MAX_STORE_SIZE) {
      // Remove oldest entry
      const oldestKey = Array.from(this.storageTimestamps.keys())
        .sort((a, b) => this.storageTimestamps.get(a) - this.storageTimestamps.get(b))[0];
      if (oldestKey) {
        this.storage.delete(oldestKey);
        this.storageTimestamps.delete(oldestKey);
      }
    ...
    this._logDebug('put - Closest nodes for key:', nodes.map(n => n.id));
    ...
    const promises = nodes.map(async node => {
      ...
      return new Promise(resolve => {
        ...
        const responseHandler = (message, sender) => {
          ...
          this._logDebug('put - STORE response from peer:', node.id, 'success:', message.success);
    ...
    this._logDebug('put - STORE operation completed. Success count:', successCount, 'out of', nodes.length);
  ...
  async get(key) {
    ...
    const keyHashHex = /^[a-fA-F0-9]{40}$/.test(key) ? key : bufferToHex(await sha1(key));
  ...
  async _replicateToNewPeer(newPeerIdHex) {
    ...
    for (const [keyHashHex, value] of this.storage.entries()) {
      ...
      if (isNewPeerClosest) {
        ...
        if (peer && peer.connected) {
          this._logDebug(`New peer ${newPeerIdHex.substring(0,4)} is close to key ${keyHashHex.substring(0,4)}, replicating...`);
          peer.send({
            type: 'STORE',
            sender: this.nodeIdHex,
            key: keyHashHex,
