    ...
    script.onload = () => {
      if (window.SimplePeer) {
        ...
        if (window.simplePeer) {
          ...
          const error = new Error('SimplePeer loaded from CDN but not available on window');
          console.error(error);
          reject(error);
        }
      }
  ...
  if (ENV.NODE) {
    ...
    if (cdnLoadingPromise) {
      ...
      try {
        ...
        throw new Error('Failed to load SimplePeer from CDN. Please check your network connection.');
      }
    }
    ...
    if (!cdnLoadingPromise) {
      ...
      if (cdnLoadingPromise) {
        try {
          ...
          throw new Error('Failed to load SimplePeer. Please refresh the page and try again.');
        }
      }
    }
    
    // If we still don't have SimplePeer, that's an error
    throw new Error('Failed to load SimplePeer from CDN. Make sure you are connected to the internet.');
  }
}
