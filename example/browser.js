...
async function initApp() {
  try {
    ...
    dht.on('ready', (nodeId) => {
      ...
      document.getElementById('status').textContent = 'DHT node created with ID: ' + nodeId;
  ...
  function updatePeerList(peerIds) {
    ...
    peerIds.forEach(peerId => {
      ...
      peerLink.onclick = (e) => {
        ...
        const targetInput = document.getElementById('targetPeerId') || document.getElementById('peerInfo');
        if (targetInput) {
    ...
    try {
      ...
      switch (data.type) {
          ...
          if (data.peerId) {
            ...
            if (peerListEl) {
              ...
              for (const child of peerListEl.children) {
                if (child.textContent && child.textContent.includes(data.peerId)) {
                  peerExists = true;
                  break;
                }
              }
              ...
              if (!peerExists) {
                updateStatus(`New peer discovered: ${data.peerId.substring(0, 8)}...`);
        ...
        case 'signal':
          ...
          console.log(`Signal from: ${data.peerId?.substring(0, 8)}...`, data.signal);
          ...
          if (!connectedPeers.has(data.peerId) && !pendingConnections.has(data.peerId)) {
            ...
            updateStatus(`Incoming connection from: ${data.peerId.substring(0, 8)}...`);
          }
          
          // Pass the signal to our DHT
          // NOTE: DHT.signal expects { id, signal } format, not { peerId, signal }
          try {
            console.log(`Processing signal from: ${data.peerId.substring(0, 8)}...`);
            dht.signal({
...
async function initiateConnection(dht, peerId) {
  ...
  try {
    ...
    peer.on('connect', () => {
      ...
      document.getElementById('status').textContent = `Connected to: ${peerId.substring(0, 8)}...`;
    });
...
function setupUI(dht) {
  ...
  dht.on('signal', data => {
    if (data && data.peerId && data.signal) {
      console.log(`Signal from DHT for peer: ${data.peerId.substring(0, 8)}...`);
  ...
  dht.on('peer:connect', peerId => {
    ...
    statusEl.textContent = 'Connected to peer: ' + peerId.substring(0, 8) + '...';
  ...
  dht.on('peer:disconnect', peerId => {
    ...
    statusEl.textContent = 'Disconnected from peer: ' + peerId.substring(0, 8) + '...';
  ...
  dht.on('peer:error', ({ peer, error }) => {
    ...
    statusEl.textContent = 'Error with peer: ' + peer.substring(0, 8) + '... - ' + error;
  });
  ...
  connectForm.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (peer) {
        statusEl.textContent = `Connected to: ${peerIdInput.substring(0, 8)}...`;
      }
    } catch (err) {
  ...
  if (sendButton && messageInput && sendPeerIdEl && messagesList) {
    sendButton.addEventListener('click', () => {
      ...
      try {
        ...
        if (!peer) {
          updateStatus(`Peer ${selectedPeerId.substring(0, 8)}... not found`, true);
          return;
        }
...
async function demonstrateSHA1() {
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      peerEl.innerHTML = `<strong>Peer ${i+1}:</strong> ${id.substring(0, 8)}...${id.substring(id.length - 8)} <button class="button button-secondary" style="font-size: 12px; padding: 2px 6px; margin-left: 10px;" onclick="navigator.clipboard.writeText('${id}')">Copy</button>`;
